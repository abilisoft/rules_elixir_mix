...
load("//private:beam_info.bzl", "ErlangAppInfo", "compile_depset", "crypto_exec_inputs", "crypto_exec_tools", "fips_erl_args", "flat_compile_deps", "otp_runtime_env", "runtime_depset")
...
def _erlang_app_impl(ctx):
    ...
    compile_deps = flat_compile_deps(ctx.attr.compile_deps + ctx.attr.runtime_deps)
    ...
    ctx.actions.run(
        ...
        arguments = [args],
        inputs = depset(
            direct = sources + app_srcs + include_files + ctx.files.priv + [config, ctx.file._driver],
            transitive = [toolchain.runtime_files, crypto_exec_inputs(toolchain.otpinfo)] + [dep[DefaultInfo].files for dep in compile_deps],
        ),

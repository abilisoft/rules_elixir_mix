...
The signed [`v0.3.3` GitHub release](https://github.com/abilisoft/rules_elixir_mix/releases/tag/v0.3.3)
...
git_override(
    ...
    commit = "<full peeled commit ID for v0.3.3>",
)
...
```console
git fetch https://github.com/abilisoft/rules_elixir_mix.git tag v0.3.3
git verify-tag FETCH_HEAD
git rev-parse FETCH_HEAD^{}
```

For a later release, replace both the declared version and commit with that
release's values. Never track a branch or a tag name in `git_override`.
Repository downloads, toolchains, and package archives must remain
content-addressed.
